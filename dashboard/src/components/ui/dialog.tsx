    ...
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          ...
          // Mobile: bottom sheet — full-width, pinned to the bottom, rounded
          // top, scrollable, and thumb-reachable. Slides up from the bottom.
          "inset-x-0 bottom-0 max-h-[92dvh] w-full overflow-y-auto rounded-t-2xl border-t p-4 data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
          // sm+: restore the centered modal dialog (unchanged desktop look).
          "sm:inset-x-auto sm:bottom-auto sm:left-[50%] sm:top-[50%] sm:max-h-none sm:max-w-lg sm:translate-x-[-50%] sm:translate-y-[-50%] sm:overflow-visible sm:rounded-lg sm:border sm:p-6 sm:data-[state=closed]:zoom-out-95 sm:data-[state=open]:zoom-in-95 sm:data-[state=closed]:slide-out-to-left-1/2 sm:data-[state=closed]:slide-out-to-top-[48%] sm:data-[state=open]:slide-in-from-left-1/2 sm:data-[state=open]:slide-in-from-top-[48%]",
          className
        )}
        {...props}
      >
        ...
        <DialogPrimitive.Close className="absolute end-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground">
          <X className="h-4 w-4" />
          <span className="sr-only">{t('common:aria.close')}</span>
