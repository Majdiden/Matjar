  ...
  if (!category) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 text-center">
        <h2 className="text-2xl font-bold mb-2">{t('theme.category.not_found_heading')}</h2>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        >
          <option value="newest">{t('theme.products.sort.newest')}</option>
          <option value="price_asc">{t('theme.products.sort.price_asc')}</option>
          <option value="price_desc">{t('theme.products.sort.price_desc')}</option>
        </select>
      </div>
        ...
        <>
          ...
          {pagination && pagination.pages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              ...
              <span className="text-sm text-gray-500">{t('theme.products.page_of', { page, total: pagination.pages })}</span>
              <button
                onClick={() => updateParam('page', String(page + 1))}
