    ...
    <div>
      ...
      <section className="relative bg-[var(--color-primary)] py-28 md:py-36 px-6 text-center overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-10 left-10 w-64 h-64 border border-[var(--color-border)] rounded-full" />
          <div className="absolute bottom-10 right-10 w-48 h-48 border border-[var(--color-border)] rounded-full" />
        ...
        <div className="max-w-2xl mx-auto relative z-10">
          <p className="text-[var(--color-border)] text-xs uppercase tracking-[0.4em] mb-4">
            {hero.eyebrow_text || 'Handmade & Authentic'}
          </p>
          <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 italic leading-tight">
            {hero.heading_line1 || 'Crafted by Hand,'}<br />{hero.heading_line2 || 'Made with Heart'}
          </h1>
          <p className="text-[var(--color-border)] mb-8 max-w-md mx-auto leading-relaxed">
            {hero.subheading || 'Discover one-of-a-kind pieces from independent makers and artisans around the world.'}
          </p>
          <div className="flex gap-3 justify-center">
            ...
            >
              {hero.primary_button_text || 'Explore Collection'}
            </Link>
            <Link
            ...
            >
              {hero.secondary_button_text || 'Browse Crafts'}
            </Link>
          </div>
        </div>
      </section>
      ...
      >
        ...
        <p className="text-gray-600 leading-relaxed text-lg">
          {philosophy.body_text || 'Every item in our collection tells a story -- of tradition, craftsmanship, and the human touch that no machine can replicate. We partner directly with artisans to bring you authentic, sustainable, and beautiful creations.'}
        </p>
      ...
      >
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold italic text-[var(--color-primary)] mb-2">{feat.heading || 'Hand-Picked Pieces'}</h2>
          ...
          <div className="text-center mt-10">
            ...
            >
              {feat.view_all_text || 'View All Pieces'}
            </Link>
          </div>
        </div>
      </section>
      ...
      >
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold italic text-[var(--color-primary)] mb-2">{spotlight.heading || 'Artisan Spotlight'}</h2>
      ...
      {catsEnabled && categories.length > 0 && (
        ...
        >
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold italic text-[var(--color-primary)] mb-2">{cats.heading || 'Browse by Craft'}</h2>
            ...
            {categories.slice(0, cats.max_categories || 4).map((cat) => (
              <Link key={cat._id} to={`/categories/${cat.slug}`} className="group text-center">
                ...
                {cats.show_product_count !== false && cat.productCount !== undefined && (
                  <span className="text-xs text-gray-400">{cat.productCount} {cats.product_count_label || 'pieces'}</span>
                )}
              </Link>
            ))}
          </div>
      ...
      >
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-3xl font-bold italic text-[var(--color-primary)]">{arrivals.heading || 'New Arrivals'}</h2>
      ...
      <section className="py-20 border-t border-b border-[var(--color-border)]">
        <div className="max-w-2xl mx-auto px-6 text-center">
          <h2 className="text-3xl font-bold italic text-[var(--color-primary)] mb-4">{news.heading || 'Handcrafted with Love'}</h2>
          <p className="text-gray-600 mb-8 leading-relaxed">
            {news.subheading || 'Join our community of craft lovers. Get early access to new collections, maker stories, and exclusive artisan offers.'}
          </p>
