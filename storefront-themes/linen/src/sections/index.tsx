...
const wrap = (s: Record<string, any>) => ({
  '--linen-pt': s.padding_top != null ? `${s.padding_top}px` : '64px',
  '--linen-pb': s.padding_bottom != null ? `${s.padding_bottom}px` : '64px',
 ...
 * theme's own translations supply the demo copy, so a fresh install reads in
...
const HeroSection: React.FC<SectionComponentProps> = ({ id, section }) => {
  ...
  return (
    ...
    >
        ...
        {slides.map((slide, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="relative flex items-center px-4 pb-6 pt-2 md:px-14 md:pb-32 md:pt-16 lg:px-20">
                {active && (
                  <div key={`cap-${idx}`} className="linen-caption max-w-lg">
                    {(() => {
                      ...
                      return (
                        <>
                          {eyebrow && <p className="linen-eyebrow text-clay">{eyebrow}</p>}
                          {heading && <h1 className="font-heading mt-4 text-[2rem] leading-[1.08] text-ink sm:text-5xl lg:text-[3.4rem]">{heading}</h1>}
                          {body && <p className="mt-5 max-w-md text-base text-dune md:text-lg">{body}</p>}
                          {cta && <Link to={b.cta_url || '/products'} className="linen-btn linen-btn-solid mt-8">{cta} <I.arrowRight className="h-4 w-4 rtl:-scale-x-100" /></Link>}
                        </>
                      );
                    })()}
                  </div>
      ...
      {slides.length > 1 && (
        <div className="pointer-events-none relative z-20 md:absolute md:inset-x-0 md:bottom-0">
          <div className="mx-auto flex max-w-[1280px] items-center justify-between px-4 pb-5 md:px-14 md:pb-6 lg:px-20">
            {s.show_dots !== false ? (
              <div className="pointer-events-auto flex items-center gap-2" role="tablist" aria-label={t('theme.section.hero.slides')}>
                {slides.map((_, i) => (
                  <button key={i} type="button" role="tab" aria-selected={i === idx} aria-label={t('theme.section.hero.go_to', { n: i + 1 })} onClick={() => go(i)} className="grid h-11 place-items-center px-1"><span className={`block h-2 rounded-full transition-all duration-300 ${i === idx ? 'w-8 bg-bronze' : 'w-2 bg-black/60'}`} /></button>
                ))}
              </div>
            ) : <span />}
            {s.show_arrows !== false && (
              <div className="pointer-events-auto flex gap-2">
                <button type="button" onClick={() => go(idx - 1)} className="grid h-11 w-11 place-items-center rounded-full border border-bronze bg-cream text-clay transition-colors duration-200 hover:bg-bronze hover:text-cream" aria-label={t('theme.section.hero.prev')}><I.chevronLeft className="h-5 w-5 rtl:-scale-x-100" /></button>
                <button type="button" onClick={() => go(idx + 1)} className="grid h-11 w-11 place-items-center rounded-full border border-bronze bg-cream text-clay transition-colors duration-200 hover:bg-bronze hover:text-cream" aria-label={t('theme.section.hero.next')}><I.chevronRight className="h-5 w-5 rtl:-scale-x-100" /></button>
              </div>
            )}
          </div>
          {autoplay && !paused && (
...
const ProductGridSection: React.FC<SectionComponentProps> = ({ id, onQuickView }) => {
  ...
  return (
    <section className="linen-section" style={wrap(s)}>
      <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
        <SectionHead
          eyebrow={copy(t, s.eyebrow, `theme.section.product_grid.${id}.eyebrow`, 'theme.section.product_grid.eyebrow')}
          heading={copy(t, s.heading, `theme.section.product_grid.${id}.heading`, 'theme.section.product_grid.heading')}
          align="start"
          action={(() => {
            const label = copy(t, s.view_all_text, 'theme.section.product_grid.view_all');
            return label ? <Link to={s.view_all_url || '/products'} className="linen-eyebrow inline-flex items-center gap-2 border-b border-ink pb-1 text-ink transition-colors hover:border-bronze hover:text-clay">{label} <I.arrowRight className="h-4 w-4 rtl:-scale-x-100" /></Link> : null;
          })()}
        />
        {/* Phones: a scroll-snap rail of large cards (~72vw); md+: the 4-up grid. */}
        <div className="linen-rail -mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-2 md:mx-0 md:grid md:grid-cols-4 md:gap-x-6 md:gap-y-10 md:overflow-visible md:px-0 md:pb-0">
          {loading
            ? Array.from({ length: limit }).map((_, i) => <div key={i} className="w-[72vw] shrink-0 snap-start md:w-auto"><Skeleton className="aspect-square" /><Skeleton className="mt-4 h-5 w-3/4" /><Skeleton className="mt-2 h-4 w-1/3" /></div>)
            : products.slice(0, limit).map((p: any, i: number) => (
              <Reveal key={p._id} delay={(i % 4) as 0 | 1 | 2 | 3} className="w-[72vw] shrink-0 snap-start md:w-auto">
  ...
  return (
    <section className="linen-section relative flex items-center justify-center overflow-hidden" style={{ ...wrap(s), minHeight: `${Number(s.min_height) || 520}px` }}>
      ...
      <Reveal className="relative mx-4 my-16 max-w-xl bg-cream px-8 py-12 text-center shadow-[0_30px_60px_-30px_rgba(15,15,15,0.35)] sm:px-14">
        {heading && <h2 className="font-heading text-3xl leading-tight text-ink sm:text-4xl">{heading}</h2>}
        {body && <p className="mt-4 text-dune">{body}</p>}
        {cta && <Link to={s.cta_url || '/products'} className="linen-btn linen-btn-dark mt-8">{cta}</Link>}
      </Reveal>
    </section>
  );
};
...
const CategoryMasonrySection: React.FC<SectionComponentProps> = ({ id }) => {
    ...
    <Reveal delay={(i % 4) as 0 | 1 | 2 | 3} className={`group relative overflow-hidden bg-sand ${tall ? 'row-span-2 min-h-[420px]' : 'min-h-[200px]'}`}>
      <Link to={`/categories/${cat.slug}`} className="block h-full w-full">
        ...
        <div className="absolute inset-x-0 bottom-0 p-5 text-cream sm:p-6">
          ...
          <span className="linen-eyebrow mt-3 inline-flex items-center gap-2 border-b border-white/60 pb-0.5 text-[0.68rem] text-cream transition-colors group-hover:border-cream">{copy(t, s.cta_text, 'theme.section.masonry.cta')} <I.arrowRight className="h-3.5 w-3.5 rtl:-scale-x-100" /></span>
        </div>
      </Link>
    </Reveal>
  );
  return (
    <section className="linen-section" style={wrap(s)}>
      <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
        <SectionHead eyebrow={copy(t, s.eyebrow, 'theme.section.masonry.eyebrow')} heading={copy(t, s.heading, 'theme.section.masonry.heading')} />
        {loading ? (
...
const EditorialSplitSection: React.FC<SectionComponentProps> = ({ id }) => {
  ...
  const eyebrow = copy(t, s.eyebrow, 'theme.section.editorial.eyebrow');
  const heading = copy(t, s.heading, 'theme.section.editorial.heading');
  const body = copy(t, s.body, 'theme.section.editorial.body');
  const cta = copy(t, s.cta_text, 'theme.section.editorial.cta_text');
  return (
    <section className="linen-section" style={wrap(s)}>
      <div className={`mx-auto grid max-w-[1280px] items-center gap-10 px-4 sm:px-6 md:grid-cols-2 md:gap-16`}>
        ...
        <Reveal delay={1} className="max-w-md">
          {eyebrow && <p className="linen-eyebrow text-clay">{eyebrow}</p>}
          {heading && <h2 className="font-heading mt-3 text-3xl leading-tight text-ink sm:text-4xl">{heading}</h2>}
          {body && <p className="mt-5 text-dune">{body}</p>}
          {cta && <Link to={s.cta_url || '/products'} className="linen-btn linen-btn-outline mt-8">{cta}</Link>}
        </Reveal>
      </div>
    </section>
  );
...
const TestimonialsSection: React.FC<SectionComponentProps> = ({ id, section }) => {
  ...
  return (
    <section className="linen-section bg-tint" style={wrap(s)}>
      <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
        <SectionHead eyebrow={copy(t, s.eyebrow, 'theme.section.testimonials.eyebrow')} heading={copy(t, s.heading, 'theme.section.testimonials.heading')} />
...
const StoriesSection: React.FC<SectionComponentProps> = ({ id, section }) => {
  ...
  return (
    <section className="linen-section" style={wrap(s)}>
      <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
        <SectionHead eyebrow={copy(t, s.eyebrow, 'theme.section.stories.eyebrow')} heading={copy(t, s.heading, 'theme.section.stories.heading')} />
            ...
            return (
              <Reveal key={b.id || i} delay={(i % 4) as 0 | 1 | 2 | 3} as="article" className="group">
                <Link to={v.link_url || '/'} className="relative block aspect-[4/3] overflow-hidden bg-sand">
                  ...
                  {copy(t, v.date, `theme.section.stories.${b.id}.date`) && <span className="linen-eyebrow absolute start-4 top-4 bg-cream px-3 py-1.5 text-[0.65rem] text-ink">{copy(t, v.date, `theme.section.stories.${b.id}.date`)}</span>}
                </Link>
                <h3 className="font-heading mt-5 text-2xl leading-snug text-ink"><Link to={v.link_url || '/'} className="transition-colors hover:text-clay">{copy(t, v.title, `theme.section.stories.${b.id}.title`)}</Link></h3>
                {copy(t, v.excerpt, `theme.section.stories.${b.id}.excerpt`) && <p className="mt-2 text-[0.95rem] text-dune">{copy(t, v.excerpt, `theme.section.stories.${b.id}.excerpt`)}</p>}
...
const InstagramSection: React.FC<SectionComponentProps> = ({ id, section }) => {
  ...
  const handle = copy(t, s.handle, 'theme.section.instagram.handle');
  if (!items.length) return null;
  return (
    <section className="linen-section" style={wrap(s)}>
      <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
        <SectionHead eyebrow={copy(t, s.eyebrow, 'theme.section.instagram.eyebrow')} heading={handle} action={
          s.handle_url ? <a href={s.handle_url} target="_blank" rel="noopener noreferrer" className="linen-eyebrow inline-flex items-center gap-2 text-clay"><I.instagram className="h-4 w-4" /> {handle}</a> : null
        } />
      </div>
