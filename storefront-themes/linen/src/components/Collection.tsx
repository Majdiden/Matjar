...
export const Collection: React.FC<Props> = ({ categorySlug, title, bannerImage, description }) => {
  ...
  const query = useMemo(() => {
    ...
    if (paramCategory && !categorySlug) q.category = paramCategory;
    return q;
  }, [sort, search, minPrice, maxPrice, paramCategory, categorySlug]);
  ...
  }, [JSON.stringify(query), categorySlug]);
  ...
  const Group: React.FC<{ id: string; label: string; children: React.ReactNode }> = ({ id, label, children }) => (
    <div className="border-b border-line py-4">
      <button type="button" onClick={() => setOpen((o) => ({ ...o, [id]: !o[id] }))} aria-expanded={!!open[id]} className="flex w-full items-center justify-between py-1 text-start">
        <span className="linen-eyebrow text-ink">{label}</span>
        <I.chevronDown className={`h-4 w-4 text-dune transition-transform duration-300 ${open[id] ? 'rotate-180' : ''}`} />
      </button>
  ...
  const filters = (
    <div>
      ...
      <Group id="availability" label={t('theme.collection.availability')}>
        <label className="flex cursor-pointer items-center gap-3 text-[0.95rem] text-ink">
          <input type="checkbox" checked={inStockOnly} onChange={(e) => set({ stock: e.target.checked ? 'in' : null })} className="h-4 w-4 accent-[color:var(--color-primary)]" />
          {t('theme.collection.in_stock')}
        </label>
      </Group>
      <Group id="price" label={t('theme.collection.price')}>
        <form className="flex items-center gap-2" onSubmit={(e) => { e.preventDefault(); set({ minPrice: priceDraft.min || null, maxPrice: priceDraft.max || null }); }}>
          ...
          <button type="submit" className="linen-btn linen-btn-dark h-10 px-3 py-0 text-[0.65rem]">{t('theme.collection.apply')}</button>
        </form>
      </Group>
      {!categorySlug && rootCats.length > 0 && (
        <Group id="category" label={t('theme.collection.category')}>
              ...
              return (
                <li key={c._id}>
                  <div className="flex items-center justify-between">
                    <button type="button" onClick={() => set({ category: on ? null : c._id })} className={`py-1 text-start text-[0.95rem] transition-colors hover:text-clay ${on ? 'font-bold text-clay' : 'text-ink'}`}>{c.name}</button>
                    {kids.length > 0 && <button type="button" onClick={() => setOpen((o) => ({ ...o, [`cat-${c._id}`]: !o[`cat-${c._id}`] }))} aria-label={c.name} className="grid h-7 w-7 place-items-center text-dune hover:text-ink">{open[`cat-${c._id}`] ? <I.minus className="h-3.5 w-3.5" /> : <I.plus className="h-3.5 w-3.5" />}</button>}
                  </div>
  ...
  return (
    <div>
      ...
      <div className={`fixed inset-0 z-[120] lg:hidden ${filtersOpen ? '' : 'pointer-events-none'}`} aria-hidden={!filtersOpen}>
        ...
        <div ref={drawer} role="dialog" aria-modal="true" aria-label={t('theme.collection.filter')} className={`linen-drawer linen-scroll absolute inset-y-0 start-0 w-[88vw] max-w-sm overflow-y-auto bg-cream px-5 py-5 ${filtersOpen ? 'translate-x-0' : 'ltr:-translate-x-full rtl:translate-x-full'}`}>
          <button type="button" onClick={() => setFiltersOpen(false)} className="mb-2 ms-auto grid h-10 w-10 place-items-center text-ink" aria-label={t('theme.collection.close')}><I.close className="h-5 w-5" /></button>
          {filters}
          <button type="button" onClick={() => setFiltersOpen(false)} className="linen-btn linen-btn-dark mt-6 w-full">{t('theme.collection.show_results', { count: total })}</button>
        </div>
      </div>
