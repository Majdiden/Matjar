...
import {
  ...
  useFeaturedProducts,
  useProducts,
} from '@matjar/theme-shared/hooks/useProducts';
import { useCollections } from '@matjar/theme-shared/hooks/useCollections';
...
export const HeroShowcaseSection: React.FC<SectionComponentProps> = ({ id }) => {
  ...
  // Spotlight slider — customisable from the dashboard via section settings:
  //   product_source     : 'featured' | 'newest'  (which products feed it)
  //   slide_count        : how many products to cycle (1–8, default 3)
  //   autoplay           : auto-advance (default true)
  //   autoplay_interval  : ms between slides (default 5000)
  const source: 'featured' | 'newest' = s.product_source === 'newest' ? 'newest' : 'featured';
  const slideCount = Math.max(1, Math.min(8, Number(s.slide_count) || 3));
  const { products: featured } = useFeaturedProducts(slideCount);
  const { products: newest } = useProducts({ sort: 'newest', limit: slideCount });
  const slides = (source === 'newest' ? newest : featured).slice(0, slideCount);

  const autoplay = s.autoplay !== false;
  const interval = Math.max(2000, Number(s.autoplay_interval) || 5000);
  const [active, setActive] = React.useState(0);
  // Keep the active index valid as products stream in.
  React.useEffect(() => { if (active > slides.length - 1) setActive(0); }, [slides.length, active]);
  React.useEffect(() => {
    if (!autoplay || slides.length < 2) return;
    const tid = setInterval(() => setActive((i) => (i + 1) % slides.length), interval);
    ...
    <section className="relative" style={{ backgroundColor: 'var(--color-background)' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 grid gap-4 lg:grid-cols-[240px_1fr_260px]">
        ...
        >
            ...
            <div className="relative hidden md:flex items-center justify-center">
              {spotlight ? (
                <Link to={`/products/${spotlight.slug}`} className="block relative">
                  ...
                  />
                  {/* key on the slide id so each advance cross-fades in */}
                  <img
                    key={spotlight._id}
                    src={spotlight.images?.[0] || 'https://placehold.co/400x400/eef4ff/0f172a?text=Product'}
                    alt={spotlight.name}
                    onError={(e) => { e.currentTarget.src = 'https://placehold.co/400x400/eef4ff/0f172a?text=Product'; }}
                    className="relative max-h-[300px] w-auto object-contain drop-shadow-xl animate-[fadeIn_.4s_ease-out]"
                  />
                </Link>
              ) : (
          ...
          {/* Slider dot pagination — real, clickable, reflects the slides */}
          {slides.length > 1 && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1.5">
              {slides.map((p, i) => (
                <button
                  ...
                  type="button"
                  onClick={() => setActive(i)}
                  aria-label={t('theme.hero.showcase.go_to_slide', { n: i + 1, defaultValue: `Go to slide ${i + 1}` })}
                  aria-current={i === active}
                  className="h-2.5 rounded-full transition-all duration-300"
                  style={
                    i === active
                      ? { width: '1.5rem', backgroundColor: 'var(--color-primary)' }
                      : { width: '0.625rem', backgroundColor: 'color-mix(in srgb, var(--color-foreground) 20%, transparent)' }
                  }
                />
              ))}
            </div>
