    ...
    <div className="group relative bg-white border-2 border-black overflow-hidden hover:border-[var(--color-primary)] transition">
      ...
      <div className="relative">
        <Link to={`/products/${slug}`} className="block">
          <div className="relative aspect-square overflow-hidden">
            ...
            {/* Wishlist heart — nutreko's bold rounded treatment */}
            <div className="absolute bottom-3 end-3 z-10">
              <WishlistButton
                productId={product._id || product.id}
                product={product}
                className="w-9 h-9 rounded-full bg-white shadow flex items-center justify-center transition hover:scale-110"
                renderIcon={(active) => (
            ...
            {!onSale && !isPreorder && product.isNew && (
              <div className="absolute top-0 start-0">
                ...
                >
                  {t('theme.product_card.new', { defaultValue: 'NEW' })}
                </span>
              </div>
            )}
      ...
      <div className="p-5 border-t-2 border-black">
        ...
        <div className="flex items-center gap-1.5 mt-2">
          <div className="flex">
            {[1, 2, 3, 4, 5].map((i) => (
              <svg key={i} className={`w-3 h-3 ${i <= Math.round(rating) ? '' : 'opacity-20'}`} fill={DARK} viewBox="0 0 24 24">
