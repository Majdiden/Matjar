            ...
            <div className="flex items-center gap-4 md:gap-5">
              ...
              <Link to="/wishlist" aria-label="Wishlist" className="relative hover:text-[var(--color-primary)] hidden md:block">
                ...
                {wishlistItems.length > 0 && (
                  <span className="absolute -top-2 -right-2 min-w-[18px] h-[18px] px-1 text-black text-[10px] rounded-full flex items-center justify-center font-black" style={{ backgroundColor: LIME }}>
                    {wishlistItems.length}
                  </span>
                )}
              </Link>
              <Link to="/account" aria-label="Account" className="hover:text-[var(--color-primary)] hidden md:block">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
