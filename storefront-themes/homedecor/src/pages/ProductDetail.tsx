...
const ProductDetail: React.FC = () => {
  ...
  // Pre-order resolution (centralised helper). Variant overrides
  // product. When the merchant flags a product as pre-order, the CTA
  // says "Pre-order" regardless of on-hand stock; the `maxUnits -
  // unitsReserved` counter then governs sold-out state. Deposit,
  // discount and policy note labels are surfaced below the CTA.
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <nav className="text-sm text-gray-500 mb-6">
        <Link to="/" className="hover:text-gray-700">{t('theme.nav.home', { defaultValue: 'Home' })}</Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div>
          ...
          {requiresVariantSelection ? (
            ...
            <div className="mb-6">
              <p className="text-sm text-amber-600">
                {shipDateLabel
                  ? t('theme.product_detail.preorder_ships', { date: shipDateLabel, defaultValue: 'Pre-order — ships {{date}}' })
                  : t('theme.product_detail.preorder', { defaultValue: 'Pre-order' })}
              </p>
              {preState.lowRemaining && preState.remaining !== null && (
                <p className="text-xs font-semibold text-amber-700 mt-1">{t('theme.product_detail.only_left', { count: preState.remaining, defaultValue: 'Only {{count}} left' })}</p>
              )}
          ...
          <div className="text-sm text-gray-500 space-y-1">
            {product.sku && <p>{t('theme.product_detail.sku', { defaultValue: 'SKU' })}: {product.sku}</p>}
