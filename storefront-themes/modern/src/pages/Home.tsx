    ...
    <div>
        ...
        >
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center text-white relative z-10">
            ...
            <h1 className="text-4xl md:text-6xl font-bold mb-4 leading-tight">
              {hero.heading || store?.name || 'Welcome'}
            </h1>
            <p className="text-lg md:text-xl opacity-90 mb-8 max-w-2xl mx-auto">
              {hero.subheading || store?.description || 'Discover amazing products at great prices'}
            </p>
          ...
          <div className="absolute top-1/2 left-10 w-72 h-72 bg-white/5 rounded-full blur-3xl -translate-y-1/2" />
          <div className="absolute bottom-0 right-10 w-96 h-96 bg-white/5 rounded-full blur-3xl translate-y-1/2" />
        </div>
        ...
        {hero.show_sale_slide !== false && (
          <div className="relative py-28 md:py-40 bg-gradient-to-br from-rose-600 to-pink-700">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center text-white relative z-10">
              ...
              <h2 className="text-4xl md:text-6xl font-bold mb-4">{hero.sale_heading || 'Season Sale'}</h2>
              <p className="text-xl opacity-90 mb-6">{hero.sale_subheading || 'Up to 50% off selected items'}</p>
              ...
              >
                {hero.sale_button_text || 'Shop the Sale'}
              </Link>
            </div>
          </div>
        )}
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-3xl font-bold">{feat.heading || 'Featured Products'}</h2>
              ...
              <ProductCard key={product._id} product={product} onQuickView={feat.show_quick_view !== false ? setQuickViewProduct : undefined}>
                ...
                <ProductCard.Body>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    {feat.show_add_to_cart !== false && <ProductCard.Actions addToCartText="Add" />}
                  </div>
                </ProductCard.Body>
              </ProductCard>
            ))}
            ...
            {newArrivals.map((product) => (
              <ProductCard key={product._id} product={product} onQuickView={setQuickViewProduct}>
                ...
                <ProductCard.Body>
                  <ProductCard.Title />
                  <ProductCard.Price />
                  {arrivals.show_add_to_cart !== false && <ProductCard.Actions addToCartText="Add to Cart" />}
                </ProductCard.Body>
              </ProductCard>
            ))}
          </Carousel>
      ...
      >
        <div className="max-w-2xl mx-auto px-4 text-center text-white">
          <h2 className="text-3xl font-bold mb-2">{news.heading || 'Stay in the Loop'}</h2>
          <p className="opacity-90 mb-8">{news.subheading || 'Subscribe for exclusive deals, new arrivals, and insider offers.'}</p>
