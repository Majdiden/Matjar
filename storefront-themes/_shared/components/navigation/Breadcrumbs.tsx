...
export function Breadcrumbs(props: BreadcrumbsProps) {
  ...
  const sep = separator || (
    <svg className="w-3 h-3 text-gray-400 rtl:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
    </svg>
  );

  return (
    <nav aria-label={t('breadcrumb.aria')} className={cn('flex items-center gap-2 text-sm', className)}>
      {items.map((item, i) => (
