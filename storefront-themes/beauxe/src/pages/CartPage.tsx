  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 text-center">
        ...
        <p className="text-gray-500 mb-6">{t('theme.cart.empty_subtext')}</p>
        <Link
          to="/products"
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold">{t('theme.cart.page_heading')}</h1>
        ...
        {cart.items.map((item) => (
          <div key={item.id} className="flex gap-4 p-4 border rounded-lg">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-500 mt-1">{t('theme.cart.price_each', { price: formatPrice(item.price), defaultValue: '{{price}} each' })}</p>
      ...
      <div className="border rounded-lg p-6">
        <div className="space-y-2 mb-4">
          ...
          <div className="flex justify-between text-sm text-gray-400">
            <span>{t('theme.cart.shipping_label')}</span>
            <span>{t('theme.cart.shipping_value')}</span>
          </div>
          <div className="flex justify-between text-sm text-gray-400">
            <span>{t('theme.cart.tax_label')}</span>
            <span>{t('theme.cart.tax_value')}</span>
          </div>
        </div>
        ...
        >{t('theme.cart.checkout_button')}</Link>
        <Link
          to="/products"
