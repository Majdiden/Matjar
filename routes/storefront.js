...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Shared with the SPA shell, which embeds this exact payload in the HTML
    // so the first paint is already the merchant's store rather than the
    // theme's manifest defaults (services/storefrontStoreInfo.js).
    res.json({
      success: true,
      data: {
        store: buildStoreInfo(tenant, {
          previewParam: typeof req.query.preview === "string" ? req.query.preview : null,
          previewTheme: getPreviewThemeSlug(req),
        }),
      },
    });
  })
